...
export class AuthGuardService {
  ...
  async isAuthorized(
    ...
    try {
      ...
      if (privilege == AuthGuardService.ANONYMOUS) {
        const anonymousAuthContext = this.buildAnonymousAuthContext(
          authContext.appId,
  ...
  private async _cachedAuthorize(
    ...
    if (cache.exists) {
      ...
      const authRawResponse = await this.clientService
        .getInterceptedClient(
          AuthGuard.buildRequestData(
            resource,
            false,
            this.buildAnonymousAuthContext(appId, tenantId),
            appId,
          ),
          request,
        )
        .authLegacy.authorize(token, tenantUserId, privilege);
      const authResponse = {
