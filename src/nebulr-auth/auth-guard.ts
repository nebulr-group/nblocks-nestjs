...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    this._debugger.log(
      `canActivate parsedRequest resource`,
    ...
    const { rawAcessToken, authHeader, authCookie } =
      AuthGuard._extractJWTFromRequest(parsedRequest);
    ...
    if (rawAcessToken && rawAcessToken.startsWith('Bearer ')) {
      ...
      try {
        this._debugger.log(
          `Found JWT in ${!!authHeader ? 'HTTP header' : ''} ${
            !!authCookie ? 'Cookie' : ''
          }`,
        );

        // We do not need to get the intercepted client since we're only going to use AuthContext helper
        authContext = await this.clientService
          .getClient()
          .auth.contextHelper.getAuthContextVerified(acessToken);
        this._debugger.log(
          'canActivate token verification result: ',
          authContext,
        );
    ...
    if (
      !parsedRequest.graphql &&
      this._authPaths.includes(parsedRequest.resource)
    ) {
      this._debugger.log(
        'canActivate request is part of whitelisted auth paths. Granting',
      );
      const anonymousUser = this.authGuardService.buildAnonymousAuthContext(
        appId,
        tenantId,
      );
    ...
    AuthGuard._setAuthDataForRequest(
      parsedRequest,
    ...
    if (NebulrAuthService.isAnonymousUser(authResponse.authContext)) {
      this._debugger.log(
        `canActivate no need to do plan restriction since this is a ANONYMOUS user`,
      );
      return true;
    } else {
      const hasRequiredPlan = this.authGuardService.hasRequiredPlan(
        authResponse.authContext.tenantPlan,
        parsedRequest.resource,
      );
  ...
  private static _extractJWTFromRequest(parsedRequest: {
    graphql: boolean;
    request: Request;
    resource: string;
  }): { rawAcessToken?: string; authHeader?: string; authCookie?: string } {
    /**
     * This is the new JWT based access token that the user obtained from auth.nblocks.cloud
     */
    const authHeader =
  ...
  private parseRequest(context: ExecutionContext): {
    graphql: boolean;
    request: Request;
    resource: string;
  } {
    if (context['contextType'] == 'graphql') {
      ...
      return {
        graphql: true,
        request,
        resource: this._getGraphqlResource(graphqlCtx),
      };
    } else {
