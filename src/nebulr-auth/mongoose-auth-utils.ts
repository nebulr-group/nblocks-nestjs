...
export class MoongooseAuthUtils {
  ...
  static setAuthLoadedQueryOptions(opts: QueryOptions, nebulrAuthService: NebulrAuthService): QueryOptions {
    return { ...opts, nebulrAuthService } as QueryOptions
  }

  static setAuthLoadedSaveOptions(opts: SaveOptions, nebulrAuthService: NebulrAuthService): SaveOptions {
   ...
   */
  static resolveAuthServiceFromQuery(query: Query<any, any>): NebulrAuthService {
   ...
   * @returns NebulrAuthService
   */
  static resolveAuthServiceFromDocument(document: Document<any>): NebulrAuthService {
  ...
  private static _resolveAuthServiceFromOptions(opts: any) {
    ...
    if (!nebulrAuthService)
      throw new Error("NebulrAuthService not available. Did you load it properly as an option when executing the query/mutation?");
