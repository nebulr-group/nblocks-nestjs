...
export class UserService {
  ...
  async list(): Promise<TenantUserResponseDto[]> {
    const users = await this._getInterceptedClient()
      .tenant(this.nebulrAuthService.getCurrentTenantId())
      .users.list();
    return users;
  }
  ...
  async setGroupMembers(
    ...
    oldUsers.forEach((oldUser) => {
      if (
        oldUser.teams.includes(groupId) &&
        !users.some((u) => u.id == oldUser.id)
      ) {
  ...
  async createUsers(userNames: string[]): Promise<TenantUserResponseDto[]> {
    ...
    for (const userName of userNames) {
      const promise = this._getInterceptedClient()
        .tenant(this.nebulrAuthService.getCurrentTenantId())
        .users.create({
          username: userName,
        });
  ...
  async sendPasswordResetLink(userId: string): Promise<boolean> {
    await this._getInterceptedClient()
      .tenant(this.nebulrAuthService.getCurrentTenantId())
      .user(userId)
      .resetPassword();
    return true;
  ...
  async updateUser(user: UserInput): Promise<TenantUserResponseDto> {
    ...
    if (
      user.role === DefaultRoles.OWNER &&
      authUser.userRole != DefaultRoles.OWNER
    )
      throw Error('Logged in user must be Owner to set another user to Owner');

    const result = await this._getInterceptedClient()
      .tenant(this.nebulrAuthService.getCurrentTenantId())
      .user(user.id)
      .update({ enabled: user.enabled, role: user.role });
    return { ...user, ...result };
  }
  ...
  async updateMe(user: MeInput): Promise<TenantUserResponseDto> {
    ...
    const result = await this._getInterceptedClient()
      .tenant(this.nebulrAuthService.getCurrentTenantId())
      .user(currentUserId)
      .update({ consentsToPrivacyPolicy, firstName, lastName, onboarded });
    return { ...user, ...result };
  }
