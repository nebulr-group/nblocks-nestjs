...
export class TenantService {
  ...
  async getTenant(): Promise<TenantResponseDto> {
    ...
    const resp = await this.clientService.getClient().tenant(tenantId).get();
    return resp;
  }

  async listTenants(): Promise<TenantResponseDto[]> {
    const resp = await this.clientService.getClient().tenants.list();
    return resp;
  }
  ...
  async updateTenant(args: UpdateTenantRequestDto): Promise<TenantResponseDto> {
    const tenantId = this.nebulrAuthService.getCurrentTenantId();
    const resp = await this.clientService.getClient().tenant(tenantId).update(args);
    return resp;
  }

  async createStripeCheckoutSession(args: StripeTenantCheckoutIdRequestDto): Promise<CheckoutResponsetDto> {
    const tenantId = this.nebulrAuthService.getCurrentTenantId();
    const resp = await this.clientService.getClient().tenant(tenantId).createStripeCheckoutSession(args);
    return resp;
  }

  async createTenant(args: CreateTenantRequestDto): Promise<TenantResponseDto> {
    const resp = await this.clientService.getClient().tenants.create(args);
    return resp;
  }

  async getCustomerPortal(): Promise<string> {
    const resp = await this.clientService.getClient().tenant(this.nebulrAuthService.getCurrentTenantId()).getStripeCustomerPortalUrl();
    return resp.url;
  }
}
